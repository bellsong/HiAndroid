...
启动优化其实就是一个任务调度器，主要解决两点：

1.如何保证时序；

2.如何控制拥堵；

可以借鉴的任务调度系统，比如Google的WorkManager，或者Spark的DAGScheduler，主要思想是 面向阶段调度（Stage-Oriented Scheduler）

阶段划分，任务编排，任务调度

线程池ThreadPoolExecutor 线程池大小 2N+1 N是cpu个数，线程池优先级
...
同工具DDMS中的TraceView来检测耗时的点在哪里，做针对的处理
...
划分启动阶段：Application onCreate()；第一个Activity启动前；第一个Activity启动完成；闲时加载onIdle（主线程MessageQueue闲时回调）；

串行到并行，分阶段分模块分优先级，保证启动任务有序高效，多节点并发执行
